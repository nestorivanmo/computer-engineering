...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// --------------------------------
		// Restricciones para que los dedos no se extiendan más
		// del movimiento natural de la mano

		if (dedo1 >= 90.0f) dedo1 = 90.0f;
		if (dedo1 <= -90.0f) dedo1 = -90.0f;

		if (dedo2 >= 0.0f) dedo2 = 0.0f;
		if (dedo2 <= -180.0f) dedo2 = -180.0f;

		if (dedo3Pulgar >= 0.0f) dedo3Pulgar = 0.0f;
		if (dedo3Pulgar <= -90.0f) dedo3Pulgar = -90.0f;
		...
		model = glm::rotate(model, glm::radians(-dedo3Pulgar), glm::vec3(0.0f, 1.0, 0.0f));
		model = glm::translate(model, glm::vec3(0.5f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.0f, 0.3f, 0.25f));
		color = glm::vec3(1.0f, 0.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
